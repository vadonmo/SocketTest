...
void CTCPServerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_REC_EDIT, m_szRecv);
	DDX_Text(pDX, IDC_SND_EDIT, m_szSend);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CTCPServerDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_ACCEPT_BTN, OnAcceptBtn)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CTCPServerDlg::OnInitDialog()
{
	...
	m_sockListen.Create(6666);//监听的端口号
...
void CTCPServerDlg::OnAccept()
{
	...
	CString tmp;
	if (m_sockListen.Accept(m_socketServer, &sockAddr, &nSockAddrLen))
	{
		tmp.Format("有客户端连接，来自%d.%d.%d.%d\r\n",(UCHAR) sockAddr.sa_data[2], 
			(UCHAR) sockAddr.sa_data[3],(UCHAR) sockAddr.sa_data[4], (UCHAR) sockAddr.sa_data[5]);
		m_szRecv += tmp;
		UpdateData(FALSE);
	}
}

void CTCPServerDlg::OnRecevice()
{
	BYTE byBuf[1024] = {0};
	int nRecvLen = 0;
...
{
	UpdateData();
	int nSendLen = m_socketServer.Send((void*) m_szSend.GetBuffer(0), m_szSend.GetLength());
	if (nSendLen > 0)
	{
		AfxMessageBox("发送成功");
	}
