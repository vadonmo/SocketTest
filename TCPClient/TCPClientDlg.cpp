...
void CTCPClientDlg::OnConnectBtn() 
{
	...
	char * lpIP = "127.0.0.1";
	m_sockConnect.Connect(lpIP, 6666);
}
...
void CTCPClientDlg::OnSend() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	int nSendLen = m_sockConnect.Send((void*) m_szSend.GetBuffer(0),m_szSend.GetLength());
	if (nSendLen > 0)
	{
		AfxMessageBox("发送成功");
	}
...
void CTCPClientDlg::OnReceive()
{
	BYTE byBuf[1024] = {0};
	int nRecvLen = 0;
	nRecvLen = m_sockConnect.Receive(byBuf, sizeof(byBuf));
	CString tmp;
	if (nRecvLen > 0)
	{
		UpdateData();
